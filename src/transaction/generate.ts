...
import { createBox, getBoxAssets, getBoxInfo, subtractAssetBalance, sumAssetBalance } from "./utils";
...
export const generateTransaction = async (
  ...
  inBoxes.forEach((box) => {
    ...
    remainingAssets = sumAssetBalance(
      remainingAssets,
      getBoxAssets(box)
    );
  });
  ...
  const txCandidate = wasm.TxBuilder.new(
    ...
    outBoxCandidates,
    currentHeight,
    wasm.BoxValue.from_i64(wasm.I64.from_str(minimumFeeConfigs.txFee.toString())),
  ...
  const serializedTx = Buffer.from(reducedTx.sigma_serialize_bytes()).toString('base64');
  const ergoTx: TransactionEIP19 = {
    reducedTx: serializedTx,
    sender: minimumFeeConfigs.feeAddress,
    inputs: inBoxes.map(input => Buffer.from(input.sigma_serialize_bytes()).toString('base64'))
  }
