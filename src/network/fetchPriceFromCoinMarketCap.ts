...
const axiosCoinMarketCap = axios.create({
  baseURL: "https://api.coinmarketcap.com/data-api/v3/cryptocurrency/market-pairs/latest",
  timeout: 8000,
});

export const fetchPriceFromCoinMarketCapInUSD = async (slug: string): Promise<number> => {
  const queryParams = {
    slug: slug,
    start: 1,
    limit: 5,
  ...
  const marketsData = await axiosCoinMarketCap.get('', { params: queryParams }).then(res => res.data)
  
  const prices: Array<Price> = []
  marketsData.data.marketPairs.forEach((pool: any) => {
    if (pool.quotes[0].volume24h) prices.push({
